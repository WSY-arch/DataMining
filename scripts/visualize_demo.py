...
def main():
    ...
    parser.add_argument("--per-cluster", type=int,
                        default=20, help="演示数据每簇样本数")
    parser.add_argument("--seed", type=int, default=42, help="随机种子")
    parser.add_argument("--output-dir", type=str,
                        default="", help="输出图表目录（不指定则只显示）")
    ...
    if args.input:
        ...
        print(
            f"🔄 生成演示数据: {args.per_cluster} 个样本/簇 × {args.k} 簇, 序列长度 {args.length}")
    ...
    print("   [1/4] 绘制聚类结果...")
    plot_path_1 = str(
        output_dir / "1_clustering_results.png") if output_dir else None
    ...
    print("   [2/4] 绘制距离矩阵热力图...")
    plot_path_2 = str(
        output_dir / "2_distance_matrix.png") if output_dir else None
    if not args.no_plot or output_dir:
        plot_distance_matrix(
    ...
    plot_path_3 = str(
        output_dir / "3_medoids_comparison.png") if output_dir else None
    if not args.no_plot or output_dir:
        plot_medoids_comparison(
            X,
            result.labels,
    ...
    print("   [4/4] 绘制聚类统计...")
    plot_path_4 = str(
        output_dir / "4_cluster_statistics.png") if output_dir else None
    if not args.no_plot or output_dir:
        plot_cluster_statistics(
            result.distance_matrix,
            result.labels,
