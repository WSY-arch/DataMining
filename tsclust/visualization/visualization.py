...
def plot_distance_matrix(
    ...
    if labels is not None:
        ...
        for boundary in cluster_boundaries:
            ax.axhline(y=boundary + 0.5, color='red',
                       linestyle='--', linewidth=1, alpha=0.5)
            ax.axvline(x=boundary + 0.5, color='red',
                       linestyle='--', linewidth=1, alpha=0.5)
...
def plot_medoids_comparison(
    ...
    for cluster_id, medoid_idx in enumerate(medoids):
        ...
        for idx in cluster_indices:
            if idx != medoid_idx:
                ax.plot(X[idx], alpha=0.2,
                        color=colors[cluster_id], linewidth=0.8)
...
def plot_cluster_statistics(
    ...
    for cluster_id in range(n_clusters):
        ...
        if len(cluster_indices) > 1:
            cluster_dist = distance_matrix[np.ix_(
        ...
        if len(other_indices) > 0:
            inter_dist = distance_matrix[np.ix_(
                cluster_indices, other_indices)]
