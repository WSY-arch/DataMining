...
def test_isolation_kernel_basic():
    ...
    print(
        f"[PASS] transform() successful - embeddings shape: {embeddings.shape}")
    ...
    assert np.allclose(np.diag(dist), 1.0 - np.diag(sim)
...
def test_k_medoids_basic():
    ...
    assert np.min(labels) >= 0 and np.max(
        labels) < 2, "Labels should be in range [0, k-1]"
...
def test_cluster_time_series_demo():
    ...
    print(
        f"       - Normalized Mutual Information (NMI): {nmi:.4f} (0-1, higher is better)")
    print(
        f"       - Adjusted Rand Index (ARI): {ari:.4f} (-1 to 1, higher is better)")
    ...
    for cluster_id in range(3):
        ...
        if len(cluster_indices) > 0:
            if len(cluster_indices) > 1:
                ...
                avg_intra_dist = 0.0
            print(
                f"       - Cluster {cluster_id}: {len(cluster_indices)} samples, avg intra-dist: {avg_intra_dist:.4f}")
...
def test_different_series_lengths():
    ...
    for length in [30, 100, 200]:
        ...
        result = cluster_time_series(
            X, k=2, n_trees=30, normalize=True, random_state=123)
        n_actual_clusters = len(np.unique(result.labels))
        print(
            f"[PASS] Length {length:3d}: clustered into {n_actual_clusters} cluster(s)")
